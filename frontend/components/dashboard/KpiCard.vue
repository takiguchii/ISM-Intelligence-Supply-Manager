...
<template>
    <div class="p-6 rounded-2xl bg-white dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800/80 shadow-sm dark:shadow-lg space-y-4 h-full">
        <div class="flex items-center justify-between">
            <span class="text-xs font-mono uppercase tracking-wider text-zinc-600 dark:text-zinc-400">{{ label }}</span>
            <span
                class="w-9 h-9 rounded-xl bg-zinc-100 dark:bg-zinc-800/80 border border-zinc-300 dark:border-zinc-700/60 flex items-center justify-center text-zinc-700 dark:text-zinc-300">
                <el-icon :size="16">
                    <component :is="icon" />
                </el-icon>
            </span>
        ...
        <div class="flex items-center gap-2 flex-wrap">
            <span class="inline-flex items-center gap-1 text-xs font-mono px-2 py-0.5 rounded-full border" :class="isUp
                ? 'text-emerald-700 dark:text-emerald-400 border-emerald-400/30 bg-emerald-400/10'
