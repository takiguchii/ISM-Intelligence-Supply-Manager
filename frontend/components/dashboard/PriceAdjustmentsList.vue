...
<template>
    <div class="p-6 rounded-2xl bg-white dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800/80 shadow-sm dark:shadow-lg space-y-4">
        <div class="flex items-center justify-between flex-wrap gap-2">
            <div>
                <h3 class="text-sm font-semibold text-zinc-900 dark:text-white">Sugestões de reajuste — Agente de Precificação</h3>
                <p class="text-xs text-zinc-500">Top {{ items.length }} itens com maior potencial de ganho</p>
            </div>
            <span
                class="inline-flex items-center gap-1.5 rounded-full border border-amber-400/30 bg-amber-400/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-amber-700 dark:text-amber-400">
                <el-icon :size="12">
                    <Opportunity />
                </el-icon> IA
            </span>
        ...
        <div class="divide-y divide-zinc-200 dark:divide-zinc-800/60">
            <div v-for="row in rows" :key="row.id" class="flex items-center justify-between gap-4 py-3 flex-wrap">
                ...
                <div class="flex items-center gap-3 text-sm shrink-0">
                    ...
                    <span class="font-semibold text-zinc-900 dark:text-white">R$ {{ price(row.to) }}</span>
                    <span class="inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-[11px] font-semibold"
                        :class="row.direction === 'up'
                            ? 'border-emerald-400/30 bg-emerald-400/10 text-emerald-700 dark:text-emerald-400'
                            : 'border-sky-400/30 bg-sky-400/10 text-sky-700 dark:text-sky-400'">
                        <el-icon :size="10">
